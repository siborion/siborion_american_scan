    ...
    :  QGLWidget()
{
    ...
    qDebug()<<"angle"<<sizeof(angle)/sizeof(double);

    quint16 sizeAngle  = sizeof(angle)/sizeof(double);
    quint16 sizeAngle2 = sizeAngle*2;

    double angle_2[sizeAngle2];
    quint16 sizeAngle4 = sizeof(angle_2)/sizeof(double)*2;
    double angle_4[sizeAngle4];

    for(quint16 i=0; i<sizeAngle; i++)
    {
        angle_2[i*2+0] = angle[i];
        angle_2[i*2+1] = (angle[i]+angle[i+1])/2;
    }

    for(quint16 i=0; i<sizeAngle2; i++)
    {
        angle_4[i*2+0] =  angle_2[i];
        angle_4[i*2+1] = (angle_2[i]+angle_2[i+1])/2;
    }
    ...
    step  = 1600 + startKonus;
    step /= razmer;
    qDebug()<<"step"<<step<<"razmer"<<razmer;
    ...
    for(double dVektor=(0); dVektor<=(127); dVektor+=0.25)
    {
...
//            curDeg = getDeg(dVektor);
            curDeg = 0 - qDegreesToRadians(angle_4[dVektor4]);
...
//            if(dVektor<0.1)
//                qDebug()<<curX<<curY<<(quint32)point;
...
void scena::drawGraf()
{
    ...
    quint16 pointOffSet = startKonus/step;
    quint16 curPoint = 0;
    for(double point=0; point<1600; point=point+step)
    {
        ...
        lMassiv[curPoint][0] = curPoint + pointOffSet;
